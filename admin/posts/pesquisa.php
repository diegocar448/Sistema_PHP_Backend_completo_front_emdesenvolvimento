  <div class="content home">
    ...
    <h1 class="location">Pesquisar <strong><?php echo $search; ?></strong> em posts <span><?php echo date('d/m/Y H:i');?></span></h1><!--/location--> 
	...
	<div class="posts">
		...
		if($_POST['search']):
			...
			$readPosts = read('posts', "WHERE titulo LIKE '%$searchL%' OR titulo LIKE '%$searchU%' OR conteudo LIKE '%$searchL%' OR conteudo LIKE '%$searchU%' ORDER BY status ASC, cadastro DESC LIMIT $inicio, $maximo"); /*vai pegar os inativos e depois vai pegar pela ordem do cadastro*/		
			...
			if(!$readPosts):
				if($page > '1'):
					...
					echo '<div class="postsnotfound" style="float:left; width:950px; margin-top:10px;">Sua pesquisa não retornou resultados. Tente outros termos!</div> <!-- aparece caso não exista nada no banco -->';
				endif;					
			else:					
